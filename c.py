...
#python c.py -d nasd_input.csv -q nasd_query.csv -od enc_input.csv -oq enc_query.csv
...
# -------------------------------------------------------------------------------------------------------
  ...
  x = layers.Conv1D(16, 4, activation="relu", padding="same", name='conv1')(input_window) # 10 dims
  #x = BatchNormalization()(x)
  x = layers.MaxPooling1D(2, padding="same", name='pool1')(x) # 5 dims
  x = layers.Conv1D(1, 4, activation="relu", padding="same", name='conv2')(x) # 5 dims
  #x = BatchNormalization()(x)

  x = layers.MaxPooling1D(2, padding="same", name='pool2')(x) # 5 dims
  x = layers.Conv1D(1, 4, activation="relu", padding="same", name='conv3')(x) # 5 dims

  encoder = layers.MaxPooling1D(2, padding="same", name='encoder')(x) # 3 dims

  # 3 dimensions in the encoded layer

  x = layers.Conv1D(1, 4, activation="relu", padding="same", name='bottle')(encoder) # 3 dims
  #x = BatchNormalization()(x)

  x = layers.UpSampling1D(2, name='up1')(x) # 6 dims
  x = layers.Conv1D(16, 1, activation='relu', name='conv4')(x) # 5 dims

  x = layers.UpSampling1D(2, name='up2')(x) # 6 dims
  x = layers.Conv1D(16, 1, activation='relu', name='conv5')(x) # 5 dims
  #x = BatchNormalization()(x)
